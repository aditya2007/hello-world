...
/**
 * http://www.newthinktank.com/2013/02/java-algorithms - By Derek Banas
 *
 * Created by Yogananda Gowda - 212590467 on 5/25/17.
 */
...
public class Heap2 {
    ...
    public void fillTheArray(int[] usrInput) {

        Data3 randomData1;

        for (int i = 0; i < usrInput.length; i++) {

            randomData1 = new Data3(usrInput[i]);
    ...
    public void minHeapTheArray(int index) {
        ...
        while (index < itemsInArray / 2) {
            ...
            if (root.key <= theHeap[minChild].key)
                break;

            System.out.println("Put Index Value " + theHeap[minChild]
                    + " in Index " + index);

            // Save the value in largest child into the top
            // index

            theHeap[index] = theHeap[minChild];
    ...
    public static void main(String args[]) {
        ...
        int[] userInput = {14,8,10,4,7,9,3,1,2,16};
        //Heap2 newHeap = new Heap2(userInput.length);
        Heap2 newHeap = new Heap2(7);

        newHeap.generateFilledArray(90);
        //newHeap.fillTheArray(userInput);
